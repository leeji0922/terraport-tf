...
# 파일 저장 람다 함수
resource "aws_lambda_function" "save_files_function" {
  filename         = "lambda_save_files.zip"
  function_name    = "${var.project_name}-function-save-files"
  role            = aws_iam_role.lambda_role.arn
  handler         = "save_files.save_files"
  runtime         = "python3.12"
  timeout         = 30
  tags = {
    Name = "${var.project_name}-lambda-save-files"
    Project = var.project_name
  }
